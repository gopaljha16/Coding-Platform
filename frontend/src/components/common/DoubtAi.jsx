...
import { Send, Copy, Check, User, Bot, Code, Sparkles, Terminal, AlertTriangle } from "lucide-react";
...
function DobutAi({ problem }) {
  ...
  const formatMessage = (text) => {
    ...
    return parts.map((part, index) => {
      if (part.startsWith('```') && part.endsWith('```')) {
        ...
        if (!isList || (bullet !== listType && !(bullet.match(/\d+\./) && listType.match(/\d+\./)))) {
          isList = true;
          listType = bullet;
          return (
            <ul key={index} className={`mt-2 mb-3 pl-5 ${bullet.match(/\d+\./) ? 'list-decimal' : 'list-disc'}`}>
              <li className="text-gray-200">{content}</li>
            </ul>
          );
        } else {
          return <li key={index} className="text-gray-200">{content}</li>;
        }
      } else if (part.match(/^\n\s*💡 /)) {
        // Tip box
        const content = part.replace(/^\n\s*💡\s+/, '');
        return (
          <div key={index} className="my-3 p-3 bg-blue-900/30 border border-blue-700/50 rounded-lg">
        ...
        const content = part.replace(/^\n\s*⚠️\s+/, '');
        return (
          <div key={index} className="my-3 p-3 bg-orange-900/30 border border-orange-700/50 rounded-lg">
            <div className="flex items-start gap-2">
        ...
        return (
          <span key={index} className="whitespace-pre-wrap text-gray-200">
            {part.split(/(\*\*.*?\*\*|__.*?__|\*.*?\*|_.*?_|~~.*?~~|`[^`]+`)/g).map((segment, i) => {
              if (segment.startsWith('**') && segment.endsWith('**')) {
                return <strong key={i} className="font-semibold text-blue-300">{segment.slice(2, -2)}</strong>;
              } else if (segment.startsWith('__') && segment.endsWith('__')) {
                return <strong key={i} className="font-semibold text-blue-300">{segment.slice(2, -2)}</strong>;
              } else if (segment.startsWith('*') && segment.endsWith('*')) {
                return <em key={i} className="italic text-purple-300">{segment.slice(1, -1)}</em>;
              } else if (segment.startsWith('_') && segment.endsWith('_')) {
                return <em key={i} className="italic text-purple-300">{segment.slice(1, -1)}</em>;
              } else if (segment.startsWith('~~') && segment.endsWith('~~')) {
                return <del key={i} className="text-gray-400">{segment.slice(2, -2)}</del>;
              } else if (segment.startsWith('`') && segment.endsWith('`')) {
                const code = segment.slice(1, -1);
                return (
                  <span key={i} className="bg-gray-800 text-orange-300 px-1 py-0.5 rounded text-sm font-mono border border-gray-700">
                    {code}
                  </span>
                );
              }
  ...
  const highlightSyntax = (code, language) => {
    ...
    const keywords = {
      javascript: ['function', 'const', 'let', 'var', 'return', 'if', 'else', 'for', 'while', 'class', 'new', 'this'],
      python: ['def', 'class', 'if', 'else', 'elif', 'for', 'while', 'return', 'import', 'from', 'as', 'try', 'except'],
      java: ['public', 'class', 'static', 'void', 'return', 'if', 'else', 'for', 'while', 'new', 'this'],
      cpp: ['#include', 'using', 'namespace', 'int', 'float', 'double', 'char', 'void', 'return', 'if', 'else', 'for', 'while', 'class', 'new', 'this'],
    };
    ...
    return code.split('\n').map((line, i) => (
      <span key={i} className="block">
        {line.replace(keywordRegex, '<span class="text-blue-400">$&</span>')
    ...
    try {
      ...
      setMessages((prev) => [
        ...prev,
        {
          role: "model",
          parts: [{ 
            text: "⚠️ **Error**\n\nSorry, I encountered an issue while generating a response. This could be due to:\n\n- Network connectivity issues\n- Server overload\n- Complex query timing out\n\nPlease try again in a moment. If the problem persists, you may want to simplify your question or break it into smaller parts."
          }],
          timestamp: new Date()
        },
      ]);
    ...
    <div className="flex flex-col h-full bg-gray-900 text-gray-100">
      ...
      <div className="border-b border-gray-800 bg-gray-900/90 backdrop-blur-sm sticky top-0 z-10">
        <div className="flex items-center justify-between p-4">
          ...
          <div className="flex items-center gap-2">
            <div className={`w-2.5 h-2.5 rounded-full ${isStreaming ? 'bg-orange-500 animate-pulse' : 'bg-green-500'}`}></div>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-6 bg-gradient-to-b from-gray-900/80 via-gray-900 to-gray-900">
        {messages.map((msg, index) => (
          <div
            key={index}
            className={`flex gap-4 ${msg.role === "user" ? "flex-row-reverse" : "flex-row"} animate-in slide-in-from-bottom-4 duration-300`}
          >
            {/* Avatar */}
            ...
            <div className={`max-w-[85%] ${msg.role === "user" ? "text-right" : "text-left"}`}>
              <div className={`rounded-xl px-4 py-3 relative group transition-all ${
                msg.role === "user" 
                    ...
                    <div className="prose prose-invert max-w-none">
                      ...
                      {msg.streaming && (
                        <span className="inline-flex items-center ml-1">
                          ...
                          <span className="inline-block w-2 h-2 bg-orange-400 rounded-full animate-pulse mr-1" style={{ animationDelay: '0.2s' }}></span>
                          <span className="inline-block w-2 h-2 bg-orange-400 rounded-full animate-pulse" style={{ animationDelay: '0.4s' }}></span>
                        </span>
                      )}
                    </div>
                  )}
              ...
              {/* Timestamp */}
              <div className={`text-xs mt-1.5 ${msg.role === "user" ? "text-blue-400/80" : "text-orange-400/80"}`}>
        ...
        <div className="max-w-4xl mx-auto">
          <form onSubmit={handleSubmit(onSubmit)} className="flex items-end gap-3">
            <div className="flex-1 relative">
              <textarea
                ref={textareaRef}
                placeholder="Ask me anything about coding (e.g., 'Explain binary search', 'Debug this Python code')..."
