...
import { ArrowLeft, Search, Edit, Plus, Code, TestTube, Settings, FileText, X, Save, Loader2 } from 'lucide-react';
...
const UpdateProblem = () => {
  ...
  const handleProblemSelect = (problem) => {
    ...
    setFormData({
      ...
      tags: problem.tags || [],
      visibleTestCases: problem.visibleTestCases && problem.visibleTestCases.length > 0 
        ? problem.visibleTestCases 
        : [{ input: '', output: '' }],
      hiddenTestCases: problem.hiddenTestCases && problem.hiddenTestCases.length > 0 
        ? problem.hiddenTestCases 
        : [{ input: '', output: '' }],
      startCode: problem.startCode && problem.startCode.length > 0 
        ? problem.startCode 
        : [{ language: 'javascript', code: '' }],
      referenceSolution: problem.referenceSolution && problem.referenceSolution.length > 0 
    ...
    switch (step) {
          ...
          <div className="space-y-6">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <textarea
                        ...
                        onChange={(e) => updateTestCase('visibleTestCases', index, 'input', e.target.value)}
                        rows="3"
                    ...
                    <div>
                      ...
                      <textarea
                        ...
                        onChange={(e) => updateTestCase('visibleTestCases', index, 'output', e.target.value)}
                        rows="3"
                        className="w-full px-3 py-2 bg-slate-800 border border-slate-600 rounded text-white text-sm focus:outline-none focus:ring-1 focus:ring-orange-500 resize-none"
            ...
            <div>
              ...
              {formData.hiddenTestCases.map((testCase, index) => (
                <div key={index} className="bg-slate-700 rounded-lg p-4 mb-4">
                  <div className="flex justify-between items-center mb-3">
                    <span className="text-sm text-gray-300">Hidden Test Case {index + 1}</span>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <textarea
                        value={testCase.output}
                        onChange={(e) => updateTestCase('hiddenTestCases', index, 'output', e.target.value)}
                        rows="3"
                        className="w-full px-3 py-2 bg-slate-800 border border-slate-600 rounded text-white text-sm focus:outline-none focus:ring-1 focus:ring-orange-500 resize-none"
                        placeholder="Expected output..."
                      />
      ...
      case 3:
        return (
          <div className="space-y-6">
            <div>
              <h3 className="text-lg font-semibold text-white mb-4">Starter Code Templates</h3>
              {formData.startCode.map((code, index) => (
                <div key={index} className="bg-slate-700 rounded-lg p-4 mb-4">
                  <div className="flex justify-between items-center mb-3">
                    <span className="text-sm text-gray-300">Template {index + 1}</span>
                  </div>
                  <div className="space-y-3">
                    <div>
                      ...
                      <select
                        value={code.language}
                        onChange={(e) => updateCodeTemplate('startCode', index, 'language', e.target.value)}
                    ...
                    <div>
                      ...
                      <textarea
                        value={code.code}
                        onChange={(e) => updateCodeTemplate('startCode', index, 'code', e.target.value)}
                        rows="6"
            ...
            <div>
              <h3 className="text-lg font-semibold text-white mb-4">Reference Solutions</h3>
              {formData.referenceSolution.map((solution, index) => (
                <div key={index} className="bg-slate-700 rounded-lg p-4 mb-4">
                  <div className="flex justify-between items-center mb-3">
                    <span className="text-sm text-gray-300">Solution {index + 1}</span>
                  </div>
                  <div className="space-y-3">
                    <div>
                      <label className="block text-xs text-gray-400 mb-1">Language</label>
                      <select
                        ...
                        onChange={(e) => updateCodeTemplate('referenceSolution', index, 'language', e.target.value)}
                        className="w-full px-3 py-2 bg-slate-800 border border-slate-600 rounded text-white text-sm focus:outline-none focus:ring-1 focus:ring-orange-500"
                      >
                        <option value="javascript">JavaScript</option>
                        <option value="python">Python</option>
                    ...
                    <div>
                      <label className="block text-xs text-gray-400 mb-1">Complete Solution</label>
                      <textarea
                        value={solution.completeCode}
                        onChange={(e) => updateCodeTemplate('referenceSolution', index, 'completeCode', e.target.value)}
                        rows="8"
                        className="w-full px-3 py-2 bg-slate-800 border border-slate-600 rounded text-white text-sm focus:outline-none focus:ring-1 focus:ring-orange-500 resize-none font-mono"
        ...
        return (
          <div className="space-y-6">
            <div className="text-center">
              ...
              <p className="text-gray-300 mb-6">Please review your problem details before submitting</p>
            </div>

            <div className="bg-slate-700 rounded-lg p-6 space-y-4">
              <div>
                <h4 className="text-lg font-semibold text-white mb-2">Problem Overview</h4>
                <div className="text-sm text-gray-300 space-y-1">
                  <p><strong>Title:</strong> {formData.title}</p>
                  <p><strong>Difficulty:</strong> {formData.difficulty}</p>
                  <p><strong>Tags:</strong> {Array.isArray(formData.tags) ? formData.tags.join(', ') : formData.tags}</p>
                  <p><strong>Visible Test Cases:</strong> {formData.visibleTestCases.length}</p>
                  <p><strong>Hidden Test Cases:</strong> {formData.hiddenTestCases.length}</p>
                  <p><strong>Code Templates:</strong> {formData.startCode.length}</p>
                  <p><strong>Reference Solutions:</strong> {formData.referenceSolution.length}</p>
                </div>
              </div>
            </div>
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-700">
      <div className="max-w-7xl mx-auto px-4 py-6">
        ...
        <div className="flex items-center justify-between mb-8">
          ...
          <div className="text-center">
            <h1 className="text-4xl font-bold text-orange-500 mb-2">Problem Management</h1>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-8 h-[calc(100vh-200px)]">
          ...
          <div className="lg:col-span-2 bg-slate-800 rounded-xl p-6 border border-slate-700">
            <div className="flex items-center gap-3 mb-6">
              ...
              <h2 className="text-xl font-semibold text-white">Problems List</h2>
            </div>
                    ...
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        ...
                        <div className="flex items-center gap-2 text-sm">
                          <span className={`px-2 py-1 rounded text-xs font-medium ${getDifficultyColorClass(problem.difficulty)}`}>
                            {problem.difficulty}
                          </span>
                          <span className="text-gray-400">•</span>
                          <span className="text-gray-400">{Array.isArray(problem.tags) ? problem.tags.join(', ') : problem.tags}</span>
                        </div>
                      </div>
            ...
            {selectedProblem ? (
              ...
              <div className="flex items-center justify-center h-full">
                <div className="text-center">
                  ...
                  <h3 className="text-xl font-semibold text-white mb-2">Select a Problem to Update</h3>
                  <p className="text-gray-400">Choose a problem from the list to start updating</p>
                </div>
              </div>
            )}
          </div>
