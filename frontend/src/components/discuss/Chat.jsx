...
import { getSocket, joinDiscussion, leaveDiscussion, emitTyping, emitStopTyping, initializeSocket } from '../../utils/socket';
...
const Chat = ({ discussionId }) => {
  ...
  const connectToChat = () => {
    ...
    console.log('Using token for socket connection:', token ? token.substring(0, 10) + '...' : 'empty');
            ...
            {messages.length === 0 ? (
              ...
              messages.map((msg, idx) => (
                <div key={idx} className={`mb-2 ${msg.userId === user?._id ? 'text-right' : 'text-left'}`}>
                  <div className={`inline-block px-3 py-1 rounded-lg ${msg.userId === user?._id ? 'bg-orange-500 text-white' : 'bg-gray-700 text-gray-200'}`}>
